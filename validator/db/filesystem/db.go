...
func NewStore(databasePath string, config *Config) (*Store, error) {
	s := &Store{
...
// Backup only exists to satisfy the interface.
...
func (s *Store) ClearDB() error {
	// Remove the slashing protection directory.
	slashingProtectionDirPath := s.slashingProtectionDirPath()
	if err := os.RemoveAll(slashingProtectionDirPath); err != nil {
		return errors.Wrapf(err, "cannot remove slashing protection directory at path %s", slashingProtectionDirPath)
	}

	// Remove the configuration file.
	configurationFilePath := s.configurationFilePath()
	if err := os.RemoveAll(configurationFilePath); err != nil {
		return errors.Wrapf(err, "cannot remove configuration file at path %s", configurationFilePath)
	}
